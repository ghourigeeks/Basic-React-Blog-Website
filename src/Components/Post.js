...
// Pagination component that renders pagination links based on the pages array.
const Pagination = ({ pages }) => (
  <nav aria-label="Pagination">
    <ul className="pagination">
      {pages.map((page) => (
        <li key={page} className="page-item">
          <Link to={`/post/${page}`} className="page-link">
            {`Page ${page}`}
          </Link>
        </li>
      ))}
...
// Main Post component with nested routing.
const Post = () => {
  const pages = [1, 2]; // List of available pages for pagination.
      ...
      <div className="container px-4 px-lg-5">
        <div className="row gx-4 gx-lg-5 justify-content-center">
          ...
          <Routes>
            ...
            {/* Route for specific pages based on the ":page" parameter */}
            <Route path="/:page" element={<PageContent page={page} />} />
          </Routes>
        </div>
      </div>
    </article>
